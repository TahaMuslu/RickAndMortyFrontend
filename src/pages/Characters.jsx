...
const Characters = () => {
    ...
    useEffect(() => {
        if (search === '') {
            ...
            const filteredData2 = initData.filter((item) => item.species.toLowerCase().includes(search.toLowerCase()));
            const filteredData3 = initData.filter((item) => item.origin?.name.toLowerCase().includes(search.toLowerCase()));
            const filteredData4 = initData.filter((item) => item.location?.name.toLowerCase().includes(search.toLowerCase()));
            // only unique values
    ...
    return (
        <Row justify={'start'}>
            ...
            {data && data.map((item) => (
                <CharacterCard key={item.id} item={item} loading={loading} />
            ))}
